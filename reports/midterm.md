...
| 2 | 나라장터 첨부 PDF/HWP | 파일 다운로드 | API 응답 내 URL | ✅ 샘플 완료 |
| 3 | 네이버 뉴스 검색 API | REST API (JSON) | developers.naver.com | ✅ 수집 완료 |
| 4 | 금융위원회(FSC) RSS | RSS 2.0 | fsc.go.kr (보도·공지·행정예고) | ✅ 수집 완료 |

### 2.2 수집 규모
- **나라장터 공고 메타 (용역)**: `420,104` 건 · 수요기관 `16,551` 곳
- **기간**: 2023-01-01 ~ 2025-11-25 (27개월, 학습 일정상 조기 중단)
- **월별 분할 JSONL**: `27` 개 (`data/raw/narajangteo/*.jsonl`)
- **나라장터 첨부 파일(샘플)**: `197` 건 → PDF 본문 173건 성공 추출
- **네이버 뉴스**: `13,765` 건 (15 키워드 × 10페이지 · 중복 제거 후)
- **금융위원회 RSS**: `30` 건 (보도 10 · 공지 10 · 행정예고 10)
